...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const onWhatsApp = async(...jids: string[]) => {
		...
		if(results) {
			const mappedResults = results.list.filter((a) => !!a.contact).map(({ contact, id, lid }) => {
				console.log('🔍 [onWhatsApp] JID:', id, 'LID:', lid, 'Contact:', contact)
				return { jid: id, exists: contact, lid }
			})
			console.log('📋 [onWhatsApp] Total results:', mappedResults.length)
